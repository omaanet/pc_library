...
This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository. Keep it aligned with the actual codebase, not older product assumptions.

## Development Commands

This project uses `pnpm`, not `npm`. The package manager is pinned in `package.json` as `pnpm@10.33.0`.
...
- Forms: React Hook Form `7.65.0` + Zod `3.24.4`
...
- Proxy protects `/profile`, `/settings`, `/read-book`, and `/add-book`
